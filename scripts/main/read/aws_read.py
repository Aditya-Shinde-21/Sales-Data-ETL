    ...
    def list_files(self, s3_client, bucket_name,folder_path):
        try:
            ...
            if 'Contents' in response:
                logger.info("Total files available in folder '%s' of bucket '%s': %s", folder_path, bucket_name, response["Contents"])
                files = [f"s3a://{bucket_name}/{obj['Key']}" for obj in response['Contents'] if
